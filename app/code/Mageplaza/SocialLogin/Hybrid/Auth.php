 ...
 * Auth class
 *
 * Auth class provide a simple way to authenticate users via OpenID and OAuth.
 *
...
class Auth {
	...
	/**
	 * Try to start a new session of none then initialize Auth
	 *
	 * Auth constructor will require either a valid config array or
	...
	/**
	 * Try to initialize Auth with given $config hash or file
	 *
	 * @param array $config Configuration array or path to a configratuion file
	 * @return void
	 * @throws Exception
	...
	public static function initialize($config) {
		...
		require_once $config["path_base"] . "ProviderAdapter.php";

		require_once $config["path_base"] . "ProviderModel.php";
		require_once $config["path_base"] . "ProviderModelOpenID.php";
		require_once $config["path_base"] . "ProviderModelOAuth1.php";
		require_once $config["path_base"] . "ProviderModelOAuth2.php";

		require_once $config["path_base"] . "User.php";
		require_once $config["path_base"] . "UserProfile.php";
		require_once $config["path_base"] . "UserContact.php";
		require_once $config["path_base"] . "UserActivity.php";
		...
		Logger::info("Enter Auth::initialize()");
		Logger::info("Auth::initialize(). PHP version: " . PHP_VERSION);
		Logger::info("Auth::initialize(). Auth version: " . Auth::$version);
		Logger::info("Auth::initialize(). Auth called from: " . Auth::getCurrentUrl());
		...
		Logger::debug("Auth initialize. dump used config: ", serialize($config));
		Logger::debug("Auth initialize. dump current session: ", Auth::storage()->getSessionData());
		Logger::info("Auth initialize: check if any error is stored on the endpoint...");
		...
		if (Error::hasError()) {
			...
			Logger::error("Auth initialize: A stored Error found, Throw an new Exception and delete it from the store: Error#$c, '$m'");
		...
		Logger::info("Auth initialize: no error found. initialization succeed.");
	}
	 ...
	 * Auth::storage()->get($key) to retrieves the data for the given key, or calling
	 * Auth::storage()->set($key, $value) to store the key => $value set.
	 *
	 * @return Storage
	 */
	...
	public static function getAdapter($providerId = null) {
		Logger::info("Enter Auth::getAdapter( $providerId )");
	...
	public static function setup($providerId, $params = null) {
		Logger::debug("Enter Auth::setup( $providerId )", $params);

		if (!$params) {
			$params = Auth::storage()->get("hauth_session.$providerId.id_provider_params");

			Logger::debug("Auth::setup( $providerId ), no params given. Trying to get the stored for this provider.", $params);
		}

		if (!$params) {
			$params = array();
			Logger::info("Auth::setup( $providerId ), no stored params found for this provider. Initialize a new one for new session");
		}

		if (is_array($params) && !isset($params["hauth_return_to"])) {
			$params["hauth_return_to"] = Auth::getCurrentUrl();
			Logger::debug("Auth::setup( $providerId ). HybridAuth Callback URL set to: ", $params["hauth_return_to"]);
		}
	...
	public static function redirect($url, $mode = "PHP") {
		Logger::info("Enter Auth::redirect( $url, $mode )");
