 ...
 *
 * Basically ProviderAdapter will create a bridge from your php
 * application to the provider api.
 *
 * Auth will automatically load ProviderAdapter and create
...
class ProviderAdapter
{
	...
	function factory($id, $params = array())
	{
		...
		$this->adapter = \Magento\Framework\App\ObjectManager::getInstance()->create('Mageplaza\SocialLogin\Hybrid\\' . $this->wrapper, [
			'providerId' => $this->id,
			'config'     => $this->config,
			'params'     => $this->params
		]);
...
//		$this->adapter = new $this->wrapper($this->id, $this->config, $this->params);
	...
	/**
	 * ProviderAdapter::login(), prepare the user session and the authentication request
	 * for index.php
	 * @return void
	 * @throw Exception
	 */
	function login()
	{
		...
		foreach (Auth::$config["providers"] as $idpid => $params) {
			Auth::storage()->delete("hauth_session.{$idpid}.hauth_return_to");
			Auth::storage()->delete("hauth_session.{$idpid}.hauth_endpoint");
			Auth::storage()->delete("hauth_session.{$idpid}.id_provider_params");
		}
		...
		if (empty(Auth::$config["base_url"])) {
			...
			$HYBRID_AUTH_URL_BASE = Auth::$config["base_url"];
		}
		...
		Auth::storage()->set("hauth_session.{$this->id}.id_provider_params", $this->params);

		// store config to be used by the end point
		Auth::storage()->config("CONFIG", Auth::$config);

		// move on
		Logger::debug("ProviderAdapter::login( {$this->id} ), redirect the user to login_start URL.");

		// redirect
		if (empty($this->params["redirect_mode"])) {
			Auth::redirect($this->params["login_start"]);
		} else {
			Auth::redirect($this->params["login_start"], $this->params["redirect_mode"]);
		}
	}
	...
	function returnToCallbackUrl()
	{
		...
		$callback_url = Auth::storage()->get("hauth_session.{$this->id}.hauth_return_to");
		...
		Auth::storage()->delete("hauth_session.{$this->id}.hauth_return_to");
		Auth::storage()->delete("hauth_session.{$this->id}.hauth_endpoint");
		Auth::storage()->delete("hauth_session.{$this->id}.id_provider_params");
	...
	{
		if (isset(Auth::$config["providers"][$id])) {
			return Auth::$config["providers"][$id];
		}
