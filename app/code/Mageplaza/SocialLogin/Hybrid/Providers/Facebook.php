 ...
 */
class Facebook extends ProviderModel {
	...
	function initialize() {
		...
		if (!class_exists('FacebookApiException', false)) {
			require_once Auth::$config["path_libraries"] . "Facebook/base_facebook.php";
			require_once Auth::$config["path_libraries"] . "Facebook/facebook.php";
		}
		...
		$this->api = new SourceFacebook(array('appId' => $this->config["keys"]["id"], 'secret' => $this->config["keys"]["secret"], 'trustForwarded' => $trustForwarded));
	...
	function loginBegin() {
		...
		foreach ($optionals as $parameter) {
			if (isset($this->config[$parameter]) && !empty($this->config[$parameter])) {
				...
				if ($parameter == "auth_type") {
					...
					Auth::storage()->set('fb_auth_nonce', $nonce);
				}
			}
		}
		...
		if (isset($this->config['force']) && $this->config['force'] === true) {
			...
			Auth::storage()->set('fb_auth_nonce', $parameters['auth_nonce']);
		}
	...
	function loginFinish() {
		...
		if (isset($_REQUEST['auth_nonce'])) {
			...
			$nonce = Auth::storage()->get('fb_auth_nonce');

			//Delete the nonce
			Auth::storage()->delete('fb_auth_nonce');
	...
	function getUserProfile() {
		...
		if (!isset($data["id"])) {
			throw new Exception("User profile request failed! {$this->providerId} api returned an invalid response: " . Logger::dumpData( $data ), 6);
		}
