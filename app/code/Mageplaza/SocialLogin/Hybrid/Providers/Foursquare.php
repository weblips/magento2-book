 ...
 */
class Foursquare extends ProviderModelOAuth2 {
	...
	function getUserProfile() {
		$data = $this->api->api("users/self", "GET", Foursquare::$apiVersion);
	...
	function getUserContacts() {
		...
		$contacts = array();
		try {
			$response = $this->api->api("users/self/friends", "GET", Foursquare::$apiVersion);
	...
	private function buildPhotoURL($prefix, $suffix) {
		if (isset($prefix) && isset($suffix)) {
			return $prefix . ((isset($this->config["params"]["photo_size"])) ? ($this->config["params"]["photo_size"]) : (Foursquare::$defPhotoSize)) . $suffix;
		}
		return ("");
	}
