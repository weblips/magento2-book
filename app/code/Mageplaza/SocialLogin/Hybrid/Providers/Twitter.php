...
class Twitter extends ProviderModelOAuth1 {
	...
	function loginBegin() {
		...
		if (isset($_REQUEST['reverse_auth']) && ($_REQUEST['reverse_auth'] == 'yes')) {
			...
			$responseObj = array('x_reverse_auth_parameters' => $stage1, 'x_reverse_auth_target' => $this->config["keys"]["id"]);
	...
	function getUserProfile() {
		...
		if (!is_object($response) || !isset($response->id)) {
			throw new Exception("User profile request failed! {$this->providerId} api returned an invalid response: " . Logger::dumpData( $response ), 6);
		}
