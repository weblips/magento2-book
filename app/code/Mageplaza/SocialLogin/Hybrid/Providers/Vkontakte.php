 ...
 */
class Vkontakte extends ProviderModelOAuth2 {
	// default permissions
	public $scope = "email";
	...
	function loginFinish() {
		...
		Auth::storage()->set("hauth_session.{$this->providerId}.user_id", $response->user_id);
		Auth::storage()
	...
	function getUserProfile() {
		...
		$params['uid'] = Auth::storage()->get("hauth_session.{$this->providerId}.user_id");
		...
		// Additional data
		$this->user->profile->email = Auth::storage()->get("hauth_session.{$this->providerId}.user_email");
